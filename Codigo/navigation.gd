	...
	for tile in traversable_tiles:
		...
		for x in range(3):
			for y in range(3):
				...
				if tile == target or not astar.has_meta(str(target_id)):
					continue
...
func _get_id_for_point(point):
	...
	var x = point.x - used_rect.pos.x
	var y = point.y - used_rect.pos.y
...
func get_path(start, end):
	...
	if not astar.has_meta(str(start_id)) or not astar.has_meta(str(end_id)):
		return null
